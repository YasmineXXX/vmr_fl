...
class VideoRecord(object):
    ...
    Args:
        ...
        row: A list with four or more elements where
             1) The first element is the path to the video(represented
             by label index) sample's frames excluding the root_datapath prefix
             2) The second element is the starting frame id of the video
             3) The third element is the inclusive ending frame id of the video
             4) The fourth element is the query sentence
             5) The fifth element is the starting frame id of the clip
             6) The sixth element is the inclusive ending frame id of the clip
             7) any following elements are labels in the case of multi-label classification
    """
...
class VideoFrameDataset(torch.utils.data.Dataset):
    ...
    def _sample_indices(self, record):
        ...
        num_segments = record.num_frames // segment_duration

        if segment_duration > 0:
            offsets = np.multiply(list(range(num_segments)), segment_duration) + np.random.randint(segment_duration, size=num_segments)
        ...
        # random sampling in that case, which will lead to repeated frames.
            ...
            offsets = np.sort(np.random.randint(record.num_frames, size=num_segments))
    ...
    def _get_val_indices(self, record):
        ...
        """
        num_segments = record.num_frames // self.segment_duration

        if record.num_frames > num_segments:
    ...
    def _get_test_indices(self, record):
        ...
        tick = self.segment_duration

        num_segments = record.num_frames // tick
    ...
    def __getitem__(self, index):
        ...
        Returns:
            ...
            1) frames_label: a list of PIL images or the result
            of applying self.transform on this list if
            self.transform is not None.
            2) query_sent: query sentence
            3) start_frame:
            4) end_frame:
            5) clip_start_frame:
            6) clip_end_frame:
        ...
        sample = {}
        sample["frames_label"] = self._get(record, segment_indices)
        sample["query_sent"] = record.query_sent
